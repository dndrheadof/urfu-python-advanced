...
def generate_hw_2_db(c: sqlite3.Connection) -> None:
    ...
    print(
        f"generate_hw_2_db> Added {records_added} records to `table_truck_with_vaccine`"
    )
...
def generate_hw_3_db(c: sqlite3.Connection) -> None:
    cursor = conn.cursor()
    ...
    print(f"generate_hw_3_db> Added {records_added} records to `table_fees`.")
    print(f"generate_hw_3_db>     number of wrong fees: {len(wrong_fees)}")


generate_hw5_sql = """
DROP TABLE IF EXISTS `table_occult_car_repair`;

CREATE TABLE `table_occult_car_repair` (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    timestamp VARCHAR(100) NOT NULL,
...
def generate_hw_5_db(c: sqlite3.Connection) -> None:
    ...
    end_date = datetime.datetime(
        year=2020, month=12, day=31, hour=23, minute=59, second=59, microsecond=0
    )
    ...
    for i in range(50000):
        ...
        event_date = datetime.datetime.fromtimestamp(ts).isoformat()

        car_number = _get_random_car_number()

        car_colour = random.choice(["чёрный", "синий", "белый", "красный"])

        car_type = random.choice(
            ["BMW", "Лада", "Hyundai", "Chevrolet", "Mercedes", "Ford"]
        )

        cursor.execute(
            """
                INSERT INTO
                    `table_occult_car_repair`(timestamp, car_number, car_colour, car_type) 
                VALUES 
                    (?, ?, ?, ?);
                """,
            (event_date, car_number, car_colour, car_type),
        )
    ...
    cursor.execute("SELECT COUNT(*) FROM `table_occult_car_repair`")
    records_added, *_ = cursor.fetchone()

    print(
        f"generate_hw_6_db> Added {records_added} records to `table_occult_car_repair`."
    )
...
def generate_hw_6_db(c: sqlite3.Connection) -> None:
    ...
    print(
        f"generate_hw_7_db> Added {records_added} records to `table_effective_manager`."
    )
...
def generate_hw_7_db(c: sqlite3.Connection) -> None:
    ...
    print(f"generate_hw_8_db> Recreated `uefa_commands` and `uefa_draw`")
...
def generate_hw_8_db(c: sqlite3.Connection) -> None:
    cursor = c.cursor()

    cursor.executescript(generate_hw8_sql)
    c.commit()
    ...
    print(f"generate_hw_9_db> Created a work schedule for {number_of_people}")
